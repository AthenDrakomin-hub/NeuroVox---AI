...
*   **🧬 快速语音克隆**: 上传 10-60 秒的音频文件或直接录音，系统自动分析声纹、语气、语速并生成 AI 模型指令。
...
### 第二步：创建语音模型
1. **上传模式**: 拖入一段目标人物的音频文件（推荐 30秒左右，清晰无背景音）。
2. **录制模式**: 点击“在线录制”，模仿目标人物说话或朗读一段文字。
3. 点击 **生成模型**，AI 将分析声纹并配置系统指令。
4. **保存模型**: 生成成功后，你可以给模型命名并保存到本地库，方便下次直接加载。
